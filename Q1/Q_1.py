...
TestingAccuracy = accuracy(Predicted_class_label, Actual_class_label)

#Traing Accuracy 
Actual_class_label0 = Training_set["Class_label"]
Actual_class_label0 = Actual_class_label0.reset_index(drop=True)  # reindexing the old dataset

Predicted_class_label0 = prediction(Training_set, training_class_dataset_name)
TrainingAccuracy = accuracy(Predicted_class_label0, Actual_class_label0)

#Validation Accuracy 
Actual_class_label2 = Validate_set["Class_label"]
Actual_class_label2 = Actual_class_label2.reset_index(drop=True)  # reindexing the old dataset

Predicted_class_label2 = prediction(Validate_set, training_class_dataset_name)
ValidationAccuracy=accuracy(Predicted_class_label2, Actual_class_label2)

print('accuracy: ', TrainingAccuracy,ValidationAccuracy,TestingAccuracy)
...
############################
n=200
x1 = list(np.linspace(-100, 100, n))
x2 = list(np.linspace(-650, 300, n))
...
for i in range(no_of_class):
    ...
    plt.scatter(x1, x2, label='Scatter plot of Data', marker='*')
...
for i in range(no_of_class):
    ...
    plt.xlim(-50, 70)
    plt.ylim(-650, 300)
    plt.plot(x1, x2,'o', label='Scatter plot of Data',color =color[i])
...
#     # fig = plt.figure()
#     # ax = plt.axes(projection='3d')
#     # ax.plot_surface(x1, x2, density_array, cmap=plt.cm.jet, rstride=1, cstride=1, linewidth=0)
...
#     plt.xlim(-50, 70)
#     plt.ylim(-650, 300)
#     plt.quiver(*mean, V[:,0], V[:,1], color=['r','b','g'],scale=7)
# plt.savefig('with_vec.png')
