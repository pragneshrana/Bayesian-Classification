...
dataset = dataset.drop(columns=['x_1']) 
dataset = dataset.drop(columns=['x_2']) 
dataset = dataset.drop(columns=['x_3']) 
# 
column = ['X_1', 'X_2', 'Class_value']
mean_file  = open("mean_file.txt","a+")
varaince_file = open("varaince_file.txt","w+")
file_result = open("q_3_1_result.txt","a+")
file_result.write('Features: 1,2,3 '+ '\t')
...
Accura = accuracy(Predicted_class_label, Actual_class_label)
features = dataset.shape
file_result.write('\t Accuracy :' + str(Accura) +'\n')
...
# ############################
# n=200
# x1 = list(np.linspace(-180, 120, n))
# x2 = list(np.linspace(-180, 120, n))
...
#     plt.quiver(*mean, V[:,0], V[:,1], color=['r','b','g'], scale=21)
